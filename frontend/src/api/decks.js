...
export const fetchPublicDecks = async (params = {}) => {
  const { page = 1, search = '', sort = 'created_at', includeOwnDecks = false } = params;
  try {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) throw new Error('Utilizador não autenticado');

    const query = new URLSearchParams({ page, limit: 20, search, sort, includeOwnDecks: includeOwnDecks.toString() }).toString();
